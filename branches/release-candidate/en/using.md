# Using terminologies in FHIR - HL7 Belgium terminology Implementation Guide (IG) v1.0.0
...
1. **Validating data against terminology**— does this`Coding`resolve, and is it in the bound value set? At instance time.
...
1. Does it**expand**?
1. Is the size**plausible**— not zero, not an entire hierarchy?
1. Are the versions reported in`expansion.parameter`the intended ones?
1. Do the relevant codes pass`$validate-code`**against the value set**, and not only against the code system?
1. Does a code taken**from**the expansion validate back**into**the value set? This catches definitions that expand to more than they accept.
