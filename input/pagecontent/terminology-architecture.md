This page is informative. It specifies the components of the terminology
architecture, their roles, the content each is authoritative for, and the
interfaces between them. It defines no conformance requirements.

<div style="max-width:100%; overflow-x:auto">
<img src="BE-Tx-Architecture.png" alt="ArchiMate diagram of the terminology architecture: the FHIR terminology ecosystem, CTC, the national terminology ecosystem and the terminology consumers, and the flows between them." style="max-width:100%; height:auto"/>
</div>
...
Distribution of SNOMED CT content, national operational terminology services and
local consultation are separate concerns, served by separate components.
...
The SNOMED CT distribution server exposes a FHIR terminology API. That API is
outside this architecture and is not to be used for operational calls.

Content of the national operational terminology server is assembled from three
sources:

* national extension and refsets, pulled from the national SNOMED CT server over
  authenticated RF2 syndication;
* BE FHIR terminology assets, published in Implementation Guides;
* international code systems.

### Federation

The national operational terminology server is a federated member of the
tx.fhir.org ecosystem, not a fallback for it. Both servers realise the same
logical FHIR terminology service.

* The federation registry records which ValueSets and CodeSystems each server is
  authoritative for.
...
Consequence for implementers: a validator or IG build points at the federated
ecosystem. Nationally governed content resolves to the national server, all other
content resolves as it otherwise would. No per-artefact choice of server is
required.
...
A ValueSet in an IG may reference a refset. The refset itself is not maintained
in the IG.
...
The choice of channel is operational. It affects freshness, licensing
obligations and build reproducibility, not the semantics of the content.

A local terminology proxy holds a manifest of the content it requires and a cache
of what it has resolved, so that routine validation does not depend on network
availability.
