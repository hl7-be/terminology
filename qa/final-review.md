# First content — final review

**hl7.fhir.be.terminology 1.0.0** · objections by [DATE] · meeting [DATE]
...
*changes* content is listed below with the ticket that decided it. The branch was audited
against its commits, so there are no unlisted changes.

**Section 1 is settled — object if you disagree, silence is agreement.** Section 2 needs one
explicit yes. Section 3 needs one answer, from one person. Reasoning is in the tickets.
...
`be-vs-patient-relationship-type` on inactivated concepts · **#42** allergy exposure route ·
**#33** document types value set · **#40** RSV, waiting on the SNOMED BE release · **#19**
terminology server credentials.
